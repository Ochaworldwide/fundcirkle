...
const BankInformationDesktop = () => {
  ...
  useEffect(() => {
    ...
    fetchAccountData();
  }, []); // Only trigger when modal is open and the modalType is "Bank Information"
  ...
  if (!accountDetails) {
    return (
      <div className=" w-[45%] bg-white rounded-lg p-5 border">
        <div className="flex flex-col space-y-36">
          <div className="flex justify-between w-full">
            <h2
              className="text-2xl font-medium text-gray-800 w-fit"
              onClick={closeModal}
            >
              Your Bank Details
            </h2>
            ...
            <div className="flex w-fit">
              <button
                className="flex items-center space-x-2 px-3 py-2 text-base font-medium text-gray-800 bg-gray-100 border rounded-lg hover:bg-gray-200"
  ...
  return (
    <div className="w-[45%] bg-white rounded-lg p-5 space-y-7 border">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-medium text-gray-800" onClick={closeModal}>
          Your Bank Details
        </h2>
      ...
      <div className="">
        {Object.entries(accountDetails).map(([key, value], index) => (
          <div
            key={index}
            className="flex justify-between items-center border-t py-5"
          >
            <span className="text-lg text-gray-600 capitalize">
              {key.replace(/([A-Z])/g, " $1")}:
            </span>
            <div className="flex items-center">
              <span className="text-lg font-[400] text-gray-600">{value}</span>
              {key !== "branch_address" &&
                key !== "account_type" &&
                key !== "bank_name" && (
                  <button
...
export default BankInformationDesktop;
